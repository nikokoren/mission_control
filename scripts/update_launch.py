...
if __name__ == "__main__":
    try:
        ...
        sys.exit(0)FRESH_HOURS = 4.0      # a result holds the screen at least this long
QUIET_HOURS = 6.0      # if the next launch is further off than this, keep showing the result
STALE_HOURS = 24.0     # but never show a result older than this
...
def process_launch_data(launch, mode_override=None):
    if not launch:
        return None
    ...
    # --- the two variable slots ---
    slot_a, slot_b = build_slots(launch, mode, description, program_description, rocket_fact)
...
def main():
    ...
    output = process_launch_data(target, mode)
    if not output:
        print("Error: failed to process launch data")
        return 0
